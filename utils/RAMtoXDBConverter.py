...
def create_table(root_elem, table):
    ...
    for field in table.fields.values():
        create_field(element, field)
    # создание ограничений
    for index in table.indices:
        create_index(element, index)
    # создание индексов
    for constraint in table.constraints:
        create_constraint(element, constraint)
        
    root_elem.append(element)


def create_field(root_elem, field):
    ...
    if field.domain is not None:
        if type(field.domain) == str:
            ...
            element.set("domain", field.domain.name)
...
def create_index(root_elem, index):
...
def create_constraint(root_elem, constraint):
